...
opt <- list(file_name = "/home/phillipl/projects/hypermutR/data/ld_seqs.fasta", ancestor = "consensus", output_name = '/tmp/bla.fasta')
...
if (!is.null(cdat$seq_hypermutants)){
  write.fasta(cdat$seq_hypermutants, names(cdat$seq_hypermutants), 
              gsub(".fasta", "_hypermutants.fasta", opt$output_name))
}
write.csv(cdat$all_mut_pos, gsub('.fasta', '_mut_pos.csv', opt$output_name), row.names=F)
